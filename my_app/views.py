...
def homepage(request):
    ...
    if selected_category:
        filtered_images = [image for image in image_url_list.values() if image['category'] == selected_category]
    else:
        filtered_images = list(image_url_list.values())

    grouped_images = list(chunk_list(filtered_images, 4))

    return render(request, 'index.html', {'grouped_images': grouped_images})
...
#     image = image_url_list.get(image_id)
